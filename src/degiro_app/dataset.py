...
class DatasetProcessor:
    ...
    def split_string(string):
        if string.startswith("Compra") or string.startswith("Venta"):
            ...
            return pd.Series(
                {"Action": None, "Number": None, "Price": None, "Currency": None}
            )

    def apply_split_string_to_col(self, col_name):
        self.data[["Action", "Number", "Price", "Currency"]] = self.data[
            col_name
        ].apply(self.split_string)
    ...
    def date_and_hour_transform(self, date_col, hour_col):
        self.data["Date"] = pd.to_datetime(
            self.data[date_col] + " " + self.data[hour_col], format="%d-%m-%Y %H:%M"
        )
